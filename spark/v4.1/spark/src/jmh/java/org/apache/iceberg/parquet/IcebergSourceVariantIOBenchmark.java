...
public class IcebergSourceVariantIOBenchmark extends IcebergSourceBenchmark {
  ...
  /**
   * Filter on element 0 of the array variant:
   * {@value}.
   */
   ...
   * @param expectPushdownWhenShredded on a shredded table, should the rowgroup filter report invocation?
   * @return the result.
   */
  private long select(final Blackhole blackhole, String projection, String where, boolean expectPushdownWhenShredded) {
   ...
   * @param expectPushdownWhenShredded on a shredded table, should the rowgroup filter report invocation?
   */
  private void selectEmptyResults(final String projection, final String where, boolean expectPushdownWhenShredded) {
    String command = toSql(projection, where);
    resetMetricsCounter();
    final Dataset<Row> ds = spark().sql(command);
    assertThat(ds.collectAsList())
        .describedAs("Result of command %s", command)
        .isEmpty();
    if (expectPushdownWhenShredded) {
      expectFilteringOfShreddedFields();
    }
   ...
   *
   * TODO: failing in spark. Needs investigation.
   */
  //@Benchmark
  ...
  /**
   * On shredded tables, assert that shredded field were scanned for filtering operations.
   * Log the count at info.
   */
  private void expectFilteringOfShreddedFields() {
    if (isShredded()) {
      final long scans = ParquetMetricsRowGroupFilter.variantPredicatesShreddedMetrics();
      LOG.info("Scanned {} shredded metrics", scans);
      assertThat(scans)
          .describedAs("Number of times rowgroup metrics of shredded fields were filtered")
          .isGreaterThan(0);
    }
  }
